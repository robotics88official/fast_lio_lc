...
void ImuProcess::set_debug_mode(const bool debug_mode)
{
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  const double &imu_beg_time = timeInSeconds(v_imu.front()->header.stamp);
  const double &imu_end_time = timeInSeconds(v_imu.back()->header.stamp);
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    if (timeInSeconds(tail->header.stamp) < last_lidar_end_time_)    continue;
    ...
    if (debug_mode_) {
      fout_imu << setw(10) << timeInSeconds(head->header.stamp) - first_lidar_time << " " << angvel_avr.transpose() << " " << acc_avr.transpose() << endl;
    }

    acc_avr     = acc_avr * G_m_s2 / mean_acc.norm(); // - state_inout.ba;

    if(timeInSeconds(head->header.stamp) < last_lidar_end_time_)
    {
      dt = timeInSeconds(tail->header.stamp) - last_lidar_end_time_;
      // dt = tail->header.stamp.toSec() - pcl_beg_time;
    }
    else
    {
      dt = timeInSeconds(tail->header.stamp) - timeInSeconds(head->header.stamp);
    }
    ...
    double &&offs_t = timeInSeconds(tail->header.stamp) - pcl_beg_time;
